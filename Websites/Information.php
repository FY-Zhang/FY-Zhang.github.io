...
session_start();
	...
	if(isset($_POST['submit'])) {
		if(isset($_SESSION['user'])) {
			...
			$Country = $row['Country'];
			$q = "insert into user(username,password,Name,Type,Episodes,Country,interest) values ('$name','','$Name1','$Type',$Episodes,'$Country','$cover')";
			$result = mysqli_query($conn,$q);
			mysqli_close($conn);
			header("refresh: 0;url = Animation.php");	/* 跳转网页 */
			exit;
		}
		else {
	...
	<body>
		...
		<div id = "like">
			<form action = "Information.php" method = "post">
				<input type = "submit" name = "submit" value = "I like it" id = "submit">
			</form>
		</div>
		<!-- Footer -->
		<div id = "footer">
